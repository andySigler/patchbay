/*
  ...
  SETUP YOUR ARDUINO TO WORK WITH THE RF12 RADIO TRANCEIVER

  This sketch is borrowed and slightly modified from the
  amazing JeeLib open-source library for JeeNodes. It allows
  you to write the ID, GROUP, and FREQUENCY onto the Atmega's
  flash memory. Saving these three things permanently will save
  you a ton of time.
  
  To use, upload this sketch with your RF12 radio attached correcty.
  Once uploaded, open the Arduino Serial Monitor at 57600 baud, and
  you should see information printed from the sketch, including the
  default ID, GROUP, and FREQUENCY. These are changed through typing
  into the Arduino Serial Monitor.
  
  What are these things you're saving?
  
    FREQUENCY:  RF12's can only speak to other's on the same FREQUENCY.
                To set to 433mhz (this is best) type "4b", and enter,
                into the Serial Monitor.
                
    GROUP:      RF12's can also only speak to other's in the same GROUP,
                from 1-255. To set to GROUP 42, you would type "42g", and
                enter, into the Serial Monitor.
    
    ID:         RF12's must have different IDs within the same GROUP,
                from 1-31. To set the ID to 5, you would type "5i", and
                enter, into the Serial Monitor.
                
  I highly recommend marking your Atmega with it's ID, so you
  can know it with just a glance. I use some white tape and a marker.
